        ...
        const cascadeSerialIds = new Set(); // 璁板綍浠ヤ覆琛屽惊鐜ā寮忓惎鍔ㄧ殑杩愯锛岀敤浜庡仠姝㈡寜閽?        let cropState = null;
        let cropDrag = null;
        let imageEditMode = 'crop';
        let editDrawState = null;
        let gridCustomMode = false;
        let gridCustomLines = []; // [{type:'h'|'v', pos:0-1}] 鐩稿鍥剧墖灏哄鐨勫垎鏁颁綅缃?        let gridCustomOrientation = 'h'; // 褰撳墠鐐瑰嚮鏀剧疆鏂瑰悜
        let gridCustomHistory = []; // 鎾ら攢鏍堬細姣忔鏀剧嚎鍓嶅揩鐓?        let gridCustomDrag = null; // {index, pointerId}
        let imageEditZoom = 1.0;
        let imageEditBaseW = 0; // zoom=1 鏃跺浘鐗囨樉绀哄搴?        let imageEditBaseH = 0;
        ...
        function providerImageModels(providerId){
            // 涓嶈蛋 providerById锛堜細 fallback 鍒扮涓€涓?provider锛岄€犳垚涓插彴锛夛紝鐩存帴鏌ョ簿纭尮閰?            const provider = apiProviders.find(p => p.id === providerId);
        ...
        async function setCanvasTitle(id, title){
            ...
            try {
                ...
                if(!res.ok) throw new Error('閲嶅懡鍚嶅け璐?);
        ...
        document.querySelectorAll('[data-image-edit-mode]').forEach(btn => {
            btn.addEventListener('click', event => {
                event.stopPropagation();
                setImageEditMode(btn.dataset.imageEditMode || 'crop');
            });
        });
        ...
        // 鍥剧墖缂栬緫鍖烘粴杞缉鏀?        document.getElementById('imageEditStage').addEventListener('wheel', event => {
            if(!cropState) return;
            event.preventDefault();
            event.stopPropagation();
            ...
            const mx = event.clientX - stageRect.left; // 榧犳爣鍦?stage 鍐呭亸绉?            const my = event.clientY - stageRect.top;
        ...
        function renderMsGenBody(node){
            ...
            if(msRatioSelect && msResolutionSelect){
                ...
                const syncMsCustomSizeControls = () => {
                    msRatioSelect.value = node.msRatio ?? 'square';
        ...
        async function runMsGenNode(nodeId, opts={}){
            ...
            // 閾捐矾涓棿鑺傜偣榛樿涓嶅垱寤?Output锛涢摼灏俱€佹墜鍔ㄥ紑鍚垨宸叉湁 Output 杩炴帴鏃舵墠杈撳嚭銆?            let out = outputForNode(node, 460);
            ...
            if(!opts.cascade){ node.running = true; render(); setTimeout(() => { node.running = false; render(); }, 2000); }
            else render();
            try {
                ...
                node.generatedOutputs = outputUrls.filter(url => url && !isVideoUrl(url));
        ...
        function openLinkCreateMenu(originId, originKind, clientX, clientY){
            ...
            const menuWidth = 190;
            const menuHeight = Math.min(360, options.length * 38 + 16);
            linkCreateMenu.style.left = `${Math.max(10, Math.min(window.innerWidth - menuWidth - 10, clientX))}px`;
            linkCreateMenu.style.top = `${Math.max(10, Math.min(window.innerHeight - menuHeight - 10, clientY))}px`;
        ...
        function openImageNodeMenu(nodeId, clientX, clientY){
            ...
            closeCreateMenu();
            imageNodeMenu.innerHTML = `<button class="menu-btn" data-image-replace="${escapeAttr(nodeId)}"><i data-lucide="image-plus" class="w-4 h-4"></i><span>鏇挎崲</span></button>`;
        ...
        function setImageEditMode(mode){
            ...
            if(imageEditMode === 'grid') refreshGridSplitPreview();
            else if(imageEditMode === 'crop') clearEditDrawing();
            else if(prevImageEditMode === 'grid') clearEditDrawing(); // 绂诲紑 grid 鏃朵富鍔ㄦ竻鎺夌敾甯冧笂娈嬬暀鐨勫垎鍓茬嚎棰勮
            refreshIcons();
        }
        ...
        function beginEditDraw(event){
            ...
            editDrawState = {x:p.x, y:p.y, pointerId:event.pointerId};
            ctx.lineCap = 'round';
            ctx.lineJoin = 'round';
            ctx.lineWidth = editBrushSize();
            ctx.strokeStyle = imageEditMode === 'mask' ? 'rgba(255,255,255,0.78)' : (document.getElementById('paintBrushColor')?.value || '#ff2d55');
        ...
        function gridSplitRectsCustom(width, height){
            ...
            const hCuts = [0, ...rawH, height]; // 鍒囧壊杈圭晫锛堝惈鍥剧墖涓ょ锛?            const vCuts = [0, ...rawV, width];
            const rects = [];
        ...
        // 鈥斺€斺€?鑷畾涔夊鏍艰緟鍔╁嚱鏁?鈥斺€斺€?        function toggleGridCustomMode(){
            gridCustomMode = !gridCustomMode;
            if(gridCustomMode){ gridCustomLines = []; gridCustomHistory = []; } // 杩涘叆鑷畾涔夋椂娓呯┖鏃х嚎鍙婂巻鍙?            gridCustomDrag = null;
        ...
        // 鈥斺€斺€?鍥剧墖缂╂斁 鈥斺€斺€?        function applyImageEditZoom(){
        ...
        async function applyImageGridSplit(){
            ...
            if(files.length){
                ...
                appendOutputImages(out, urls, {url:node.url, name:node.name || 'source image'}, urls.map(() => ({runMs:0, run:{prompt:'瀹牸鍒囧垎', refs:[{url:node.url, name:node.name || 'source image'}]}})));
                closeImageEditor();
                render();
                scheduleSave();
            }
        ...
        function captureOutputScrolls(){
            ...
            // LLM 鑱婂ぉ鏃ュ織婊氬姩浣嶇疆锛堣褰曟槸鍚﹀湪搴曢儴锛屼互渚挎仮澶嶆椂淇濇寔搴曢儴锛?            nodesEl.querySelectorAll('.llm-node').forEach(el => {
            ...
            requestAnimationFrame(() => {
                state.forEach((pos, key) => {
                    if(key.startsWith('out:')){
                        ...
                        if(log){
                            // 涔嬪墠鍦ㄥ簳閮?鈫?淇濇寔搴曢儴锛堟樉绀烘渶鏂版秷鎭級锛涘惁鍒欐仮澶嶅師浣?                            log.scrollTop = pos.atBottom ? log.scrollHeight : (pos.top || 0);
                        }
                    }
                });
            });
        ...
        function renderNode(node){
            ...
            const statusHtml = showStatus ? (() => {
                const label = { queued:'鎺掗槦涓?, running:'杩愯涓?, done:'瀹屾垚', failed:'澶辫触' }[node.runStatus] || '';
            ...
            if(node.type === 'image') {
                if(node.url) {
                    body.innerHTML = `<div class="image-preview-wrap"><img src="${escapeAttr(node.url)}" draggable="false"></div><div class="image-caption text-[11px] text-gray-400 truncate">${escapeHtml(node.name || 'image')}</div>`;
                    ...
                    body.ondragover = e => {
                        if(hasImageFiles(e.dataTransfer?.items) || hasOutputImageDrag(e.dataTransfer)){
                            ...
                            /* 鑺傜偣鍐呮嫋鎷芥椂鎶婄敾甯冨叏灞€鐨勬瘺鐜荤拑瑕嗙洊灞傚叧鎺夛紝閬垮厤 stopPropagation 鍚?board 娌℃満浼氭竻瀹?*/
                ...
                body.innerHTML = `<div class="blank-image"><i data-lucide="image-plus" class="w-7 h-7"></i><div class="text-[11px] font-bold">${tr('canvas.clickDragPasteImage')}</div></div>`;
                    ...
                    blank.ondragover = e => { if(hasImageFiles(e.dataTransfer?.items) || hasOutputImageDrag(e.dataTransfer)){ e.preventDefault(); e.stopPropagation(); e.dataTransfer.dropEffect = hasOutputImageDrag(e.dataTransfer) ? 'copy' : 'move'; blank.classList.add('drag-over'); dropOverlay.classList.remove('active'); } };
            ...
            if(node.type === 'output') {
                const pendingHtml = (node._pending || []).map(p =>
                    `<div class="output-img-wrap loading-wrap" data-pending-id="${escapeAttr(p.id)}"><span class="output-time-pill running">${formatRunDuration(nowMs() - Number(p.startedAt || nowMs()))}</span><div class="output-spinner"></div><button class="output-del" title="${tr('common.delete')}">脳</button></div>`
                ).join('');
                body.innerHTML = `<div class="output-grid">${(node.images || []).map(url => renderOutputMedia(url)).join('')}${pendingHtml}</div>`;
                body.onwheel = e => {
                    e.stopPropagation();
                };
            ...
            if(out) {
                const startOutLink = e => { if(e.button === 0 && !e.shiftKey) startLink(e, node.id, 'out'); };
                if(window.PointerEvent) out.onpointerdown = startOutLink;
                else out.onmousedown = startOutLink;
            }
            const inp = el.querySelector('.port.in');
            if(inp) {
                const startInLink = e => { if(e.button === 0 && !e.shiftKey) startLink(e, node.id, 'in'); };
                if(window.PointerEvent) inp.onpointerdown = startInLink;
                else inp.onmousedown = startInLink;
            }
            return el;
        }
        ...
        function renderLoopPrompt(node, ctx=loopContext){
            ...
            const replaceVars = text => String(text || '')
                ...
                .replaceAll('銆婃€绘暟銆?, String(total))
                .replaceAll('銆婅繘搴︺€?, `${index}/${total}`)
        ...
        function loopTokenLabel(token){
            if(token === '銆婅鏁般€?) return tr('canvas.counterToken');
            if(token === '銆婃€绘暟銆?) return tr('canvas.totalToken');
            if(token === '銆婅繘搴︺€?) return tr('canvas.progressToken');
            return token;
        }
        ...
        function renderLoopBody(node){
            ...
            const loopRunHtml = loopTargetId ? (cascadeSerialIds.has(loopTargetId)
                ? `<div class="gen-run-row"><button class="gen-cascade-btn gen-cascade-stop" type="button" data-loop-cascade-stop="${loopTargetId}"><i data-lucide="square" class="w-4 h-4"></i><span>鍋滄寰幆</span></button></div>`
                : `<div class="gen-run-row"><button class="gen-cascade-btn" type="button" data-loop-cascade="${loopTargetId}" title="浠庡綋鍓嶅惊鐜妭鐐瑰惎鍔ㄦ暣鏉″伐浣滄祦"><i data-lucide="play-circle" class="w-4 h-4"></i><span>寮€濮?${loopTargetOrder.length || 1} 涓妭鐐?脳 ${node.count} ${tr('canvas.loopRounds')}</span></button></div>`)
                : '';
            wrap.innerHTML = `
                ...
                ${node.showPrompt ? `<div class="loop-prompt-panel">
                    <div class="loop-field">
                        <div class="loop-variable-editor" contenteditable="true" data-placeholder="${escapeAttr(tr('canvas.loopVariablePlaceholder'))}">${loopVariableHtml(node.variablePrompt || '')}</div>
                    </div>
                    <div class="loop-start-row">
                        <button class="loop-token-btn loop-counter-token-btn" type="button" data-token="銆婅鏁般€?>${tr('canvas.counterToken')}</button>
            ...
            countInput.oninput = e => {
                ...
                if(loopCascadeBtn){
                    ...
                    if(span) span.textContent = `寮€濮?${loopTargetOrder.length || 1} 涓妭鐐?脳 ${node.count} ${tr('canvas.loopRounds')}`;
                }
                if(loopTargetId){
                    ...
                    if(targetCascadeBtn){
                        ...
                        if(span){
                            ...
                            span.textContent = `涓€閿繍琛?${targetOrder.length} 涓妭鐐?脳 ${node.count} ${tr('canvas.loopRounds')}`;
                        }
                    }
                }
                scheduleSave();
            ...
            wrap.querySelectorAll('[data-loop-cascade-stop]').forEach(btn => {
                ...
                btn.onclick = e => {
                    e.stopPropagation();
                    cascadeStopIds.add(btn.dataset.loopCascadeStop);
                    btn.disabled = true;
                    btn.querySelector('span').textContent = '鍋滄涓€?;
                };
            });
            return wrap;
        }
        ...
        function renderLLMNodePane(container, node){
            ...
            const inputPlaceholder = langIsEn() ? 'Type input, or connect a Prompt node鈥? : '鐩存帴杈撳叆锛屾垨杩炴帴鎻愮ず璇嶈妭鐐光€?;
            container.innerHTML = `
        ...
        function renderImageInputList(list, node, imageInputs, emptyText=null){
            ...
            imageInputs.forEach((src, i) => {
                ...
                item.innerHTML = `<span class="input-index">${i + 1}</span>${src.preview ? `<img src="${escapeAttr(src.preview)}">` : '<i data-lucide="image" class="w-6 h-6 text-slate-400"></i>'}<span class="input-label">${escapeHtml(src.label)}</span>`;
                item.ondragstart = e => {
                    e.stopPropagation();
                    internalDrag = true;
            ...
            imageInputs.forEach((src, i) => {
                ...
                item.innerHTML = `
                    <div class="video-input-thumb">
                        ...
                        ${src.preview ? `<img src="${escapeAttr(src.preview)}">` : '<i data-lucide="image" class="w-6 h-6 text-slate-400"></i>'}
        ...
        function comfyFieldKind(f){
            ...
            if(f.type === 'textarea' || /prompt|text|鎻愮ず璇峾姝ｅ悜|璐熷悜/.test(key)) return 'prompt';
            return 'setting';
        }
            ...
            wrap.innerHTML = `
                ...
                <div class="comfy-content">
                    ...
                    <div class="comfy-images ${(mode === 'text' || (mode === 'custom' && !imageFieldCount)) ? 'hidden' : ''}">
                        <div class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">${mode === 'custom' ? `Images 路 ${tr('canvas.comfyNeedImages')} ${imageFieldCount}` : 'Images'}</div>
        ...
        function renderComfyImages(list, node, imageInputs){
            ...
            imageInputs.forEach((src, i) => {
                ...
                item.draggable = true;
                item.dataset.sourceId = src.id;
                item.innerHTML = `<span class="input-index">${i + 1}</span>${src.preview ? `<img src="${src.preview}">` : '<i data-lucide="text" class="w-6 h-6 text-slate-400"></i>'}<span class="input-label">${tr('canvas.image')} ${i + 1}</span>`;
                item.ondragstart = e => {
                    e.stopPropagation();
                    internalDrag = true;
                    e.dataTransfer.effectAllowed = 'move';
        ...
        function generatorSources(gen){
            return connections.filter(c => c.to === gen.id).map(c => nodes.find(n => n.id === c.from)).filter(Boolean).map(n => {
                if(n.type === 'output' && (n.images||[]).length){
                    // 浠?output 鑺傜偣鍙栨渶鏂颁竴寮犲浘褰撲綔 reference 缁欎笅娓?                    const last = [...n.images].reverse().map(outputUrlValue).find(Boolean);
                    if(last) return {id:n.id, type:'outputImage', label:'涓婃父杈撳嚭', preview:last, refs:[{url:last, name:'output.png'}], prompt:''};
                }
                ...
                if(n.type === 'prompt') return {id:n.id, type:'prompt', label:(n.text || '鎻愮ず璇?).slice(0, 32), refs:[], prompt:n.text || ''};
                if(n.type === 'loop') {
                ...
                if(n.type === 'promptGroup') {
                    ...
                    return {id:n.id, type:'promptGroup', label:`鎻愮ず璇?${prompts.length} 涓猔, refs:[], prompt:prompts.join('\n\n')};
                }
        ...
        async function runGeneratorLegacy(genId, opts={}){
            ...
            const run = runSnapshot(gen, prompt || 'Edit the reference images.', refs);
            if(out) out._pending = [...(out._pending||[]), ...pendingIds.map(id => makePending(id, run))];
            if(!opts.cascade){ gen.running = true; render(); setTimeout(() => { gen.running = false; render(); }, 2000); }
            else render();
            try {
                const payload = {
                    prompt: prompt || 'Edit the reference images.',
                ...
                gen.generatedOutputs = images.filter(url => url && !isVideoUrl(url));
        ...
        async function runVideoNode(nodeId, opts={}){
            ...
            if(!opts.cascade){ node.running = true; render(); }
            else render();
            try {
        ...
        async function uploadCanvasUrlToComfy(url){
            const blob = await fetch(url).then(r => {
                if(!r.ok) throw new Error(langIsEn() ? 'Image read failed' : '鍥剧墖璇诲彇澶辫触');
                return r.blob();
            });
            ...
            const data = await fetch('/api/upload', {method:'POST', body:form}).then(async r => {
                if(!r.ok) throw new Error(await responseErrorMessage(r, langIsEn() ? 'Image upload to ComfyUI failed' : '鍥剧墖涓婁紶鍒?ComfyUI 澶辫触'));
                return r.json();
            });
        ...
        async function runComfyNode(nodeId, opts={}){
            ...
            try {
                ...
                node.generatedOutputs = images.filter(url => url && !isVideoUrl(url));
        ...
        async function runLLMNode(nodeId, opts={}){
            ...
            if(!input){
                if(opts.cascade) throw new Error('LLM 缂哄皯鎻愮ず璇嶈緭鍏?);
        ...
        // 鍒ゆ柇鏄笉鏄€岄摼灏俱€嶈妭鐐癸細娌℃湁涓嬫父鐢熸垚鑺傜偣锛堢洿鎺ョ浉杩炴垨缁?Output 涓浆閮界畻锛?        function isTerminalGenerator(nodeId){
        ...
        function cascadeBtnHtml(node){
            // 浠呴摼灏捐妭鐐规樉绀轰竴閿繍琛?            if(!isTerminalGenerator(node.id)) return '';
            ...
            // 浠呭湪浠ヤ覆琛屾ā寮忓惎鍔ㄧ殑杩愯涓樉绀哄仠姝㈡寜閽?            if(cascadeSerialIds.has(node.id)){
                return `<button class="gen-cascade-btn gen-cascade-stop" type="button" data-cascade-stop="${node.id}"><i data-lucide="square" class="w-4 h-4"></i><span>鍋滄寰幆</span></button>`;
            }
            return `<button class="gen-cascade-btn" type="button" data-cascade="${node.id}" title="涓€閿繍琛屾暣鏉″伐浣滄祦锛堣拷婧墍鏈変笂娓哥敓鎴愯妭鐐癸級"><i data-lucide="play-circle" class="w-4 h-4"></i><span>涓€閿繍琛?${order.length} 涓妭鐐?{suffix}</span></button>`;
        }
        function retryBarHtml(node){
            ...
            return `<div class="node-retry-bar" data-retry-bar>
                ...
                <button class="node-stop-btn" type="button" data-stop="${node.id}">鍋滄</button>
            </div>`;
        }
        function bindCascadeButtons(wrap, nodeId){
            ...
            wrap.querySelectorAll(`[data-cascade-stop="${nodeId}"]`).forEach(b => {
                b.onmousedown = e => e.stopPropagation();
                b.onclick = e => { e.stopPropagation(); cascadeStopIds.add(nodeId); b.disabled = true; b.querySelector('span').textContent = '鍋滄涓€?; };
            });
        ...
        // 鈥斺€?涓€閿繍琛岋細浠庣洰鏍囪妭鐐瑰弽鍚戣拷婧埌鎵€鏈変笂娓哥敓鎴愯妭鐐癸紝鎸夋嫇鎵戦『搴忎覆琛屾墽琛?鈥斺€?        function runCascadeNodeByType(node, opts={}){
        ...
        function computeCascadeOrder(targetId){
            ...
            function dfs(id){
                ...
                if(!node) return;
                // 鎵捐鑺傜偣鐨勪笂娓?                connections.filter(c => c.to === id).forEach(c => {
                    const from = nodes.find(n => n.id === c.from);
                    if(!from) return;
                    if(GEN_TYPES.includes(from.type)){
                        dfs(from.id);
                    } else if(from.type === 'output'){
                        // output 鑺傜偣鐨勪笂娓告槸鐢熸垚鍣?                        connections.filter(cc => cc.to === from.id).forEach(cc => {
        ...
        async function runNodeCascade(nodeId){
            ...
            if(loop?.mode === 'parallel' && totalRounds > 1){
                ...
                render();
                let done = 0;
                const tasks = Array.from({length:totalRounds}, async (_, idx) => {
                    const index = startIdx + idx * loopBatchSize;
            ...
            cascadeStopIds.delete(nodeId);
            cascadeSerialIds.delete(nodeId);
            // 鍏ㄩ儴瀹屾垚锛? 绉掑悗娓呴櫎鐘舵€佸窘绔?            setTimeout(() => {
                order.forEach(id => {
                    const n = nodes.find(x => x.id === id);
                    if(n && n.runStatus === 'done'){ n.runStatus = ''; n._cascadeIdx = ''; }
                });
        ...
        // 澶辫触閲嶈瘯锛氫粠璇ヨ妭鐐圭户缁線涓嬫父璺?        async function retryNodeAndDownstream(nodeId){
            const target = nodes.find(n => n.id === nodeId);
            if(!target) return;
            const order = computeCascadeOrder(nodeId);
            // 鍙噸璺戜粠璇ヨ妭鐐瑰紑濮嬬殑鍓╀綑閾?            const idx = order.indexOf(nodeId);
            const remain = idx >= 0 ? order.slice(idx) : [nodeId];
            try { await runOneCascadePass(remain); }
            catch(err) { render(); return; }
            setTimeout(() => {
                remain.forEach(id => {
                    const n = nodes.find(x => x.id === id);
                    if(n && n.runStatus === 'done'){ n.runStatus = ''; n._cascadeIdx = ''; }
        ...
        function cancelCascade(nodeId){
            // 绠€鍗曞疄鐜帮細鎶婂綋鍓嶈妭鐐瑰拰涓婃父 queued/failed 鐘舵€佹竻鎺夛紙涓嶈兘 abort 宸插彂鍑虹殑璇锋眰锛?            const order = computeCascadeOrder(nodeId);
            order.forEach(id => {
                const n = nodes.find(x => x.id === id);
        ...
        function renderOutputMedia(item){
            ...
            if(isVideoUrl(url)){
                return `<div class="output-img-wrap"><video src="${safe}" data-url="${safe}" preload="metadata" muted playsinline></video>${timePill}<div class="output-video-badge"><i data-lucide="play" class="w-3 h-3"></i>VIDEO</div><button class="output-del" title="${tr('common.delete')}">脳</button></div>`;
            }
            return `<div class="output-img-wrap"><img src="${safe}" data-url="${safe}" alt="generated output">${timePill}<button class="output-del" title="${tr('common.delete')}">脳</button></div>`;
        }
        ...
        function appendOutputImages(out, images, compareRef, metas=[]){
            ...
            out.images = [...(out.images || []), ...list.map((url, i) => {
                ...
                return {url, viewed:false, runMs:meta.runMs || 0, run:meta.run || null};
            })];
            if(compareRef?.url){
        ...
        function groupSelectedImages(){
            ...
            [...selected].map(id => nodes.find(n => n.id === id)).filter(Boolean).forEach(n => {
                if(n.type === 'image' || n.type === 'prompt') {
                    targetMap.set(n.id, n);
                    return;
                }
                if(n.type === 'group' || n.type === 'promptGroup') {
                    (n.items || []).map(id => nodes.find(item => item.id === id)).filter(Boolean).forEach(child => {
                        if(child.type === 'image' || child.type === 'prompt') targetMap.set(child.id, child);
                    });
                }
            });
        ...
        function selectAllGroupableNodes(){
            if(!canvas) return false;
            const ids = nodes
                .filter(n => n.type === 'image' || n.type === 'prompt' || n.type === 'group' || n.type === 'promptGroup')
                .map(n => n.id);
        ...
        function startNodeDrag(e, node){
            ...
            // 濡傛灉琚嫋鑺傜偣鍦ㄥ閫夐噷锛屾墍鏈夊叾浠栭€変腑鑺傜偣锛堝惈鍏剁粍鎴愬憳锛変竴璧风Щ鍔?            if(selected.has(dragTarget.id) && selected.size > 1){
        ...
        function startLink(e, originId, originKind){
            ...
            const finishLink = e2 => {
                ...
                if(!didConnect && originKind === 'out'){
                    if(openLinkCreateMenu(originId, originKind, e2.clientX, e2.clientY)){
                        // Menu opened.
                ...
                } else if(!didConnect && originKind === 'in'){
                    openLinkCreateMenu(originId, originKind, e2.clientX, e2.clientY);
                }
                tempLink = null;
                window.removeEventListener('mousemove', moveLink, true);
                window.removeEventListener('mouseup', finishLink, true);
                window.removeEventListener('pointermove', moveLink, true);
                window.removeEventListener('pointerup', finishLink, true);
                document.removeEventListener('mousemove', moveLink, true);
                document.removeEventListener('mouseup', finishLink, true);
                document.removeEventListener('pointermove', moveLink, true);
                document.removeEventListener('pointerup', finishLink, true);
                window.onmousemove = null;
                window.onmouseup = null;
                renderLinks();
            };
            window.onmousemove = moveLink;
            window.onmouseup = finishLink;
            window.addEventListener('mousemove', moveLink, true);
            window.addEventListener('mouseup', finishLink, true);
            window.addEventListener('pointermove', moveLink, true);
            window.addEventListener('pointerup', finishLink, true);
            document.addEventListener('mousemove', moveLink, true);
            document.addEventListener('mouseup', finishLink, true);
            document.addEventListener('pointermove', moveLink, true);
            document.addEventListener('pointerup', finishLink, true);
        }
        ...
        function endDrag(event=null){
            ...
            document.body.classList.remove('canvas-node-drag', 'canvas-node-resize', 'canvas-selecting');
            window.onmousemove = null;
            window.onmouseup = null;
        ...
        window.addEventListener('keydown', e => {
            ...
            if((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'v') {
                const tag = document.activeElement?.tagName;
                if(tag === 'INPUT' || tag === 'TEXTAREA' || document.activeElement?.isContentEditable) return;
                if(clipboard?.length) { e.preventDefault(); pasteNodes(); }
            }
